...
首先锁对象是无锁状态。如果java设置开启了偏向锁。那么从无锁升级为偏向锁。

**偏向锁：**当前线程会使用判断锁对象的**MarkWord中存的是否是自己的线程ID。**

- **如果是自己的线程ID**，那么已经获取偏向锁了，直接操作资源。
- **如果不是自己的线程ID**，那么使用CAS来替换对象锁中的MarkWord为自己的线程ID。
  - ​	**如果成功**，那么获取偏向锁成功
  - ​    **如果失败**，锁升级为轻量级锁。通知拥有偏向锁的线程释放锁，然后竞争轻量级锁

偏向锁的撤销：偏向锁使用的是**一种等到竞争出现才会释放锁的机制，**所以当其他线程尝试竞争锁后，持有锁的线程会撤销锁。撤销锁的流程

1. 在**安全点**上停止持有锁的线程
2. 遍历线程栈，如果存在锁记录的话，需要修复锁记录和MarkWord，使其变成无锁状态
3. 唤醒被停止的线程，将当前锁升级成轻量级锁。

**轻量级锁：**JVM会**在自己的调用栈中创建一个存储锁记录的空间（Lock Record），**然后把锁对象的Mark Word复制到自己的锁空间中，称之为**Displaced MarkWord**。然后使用CAS尝试将锁对象的MarkWord替换为指向自己线程锁记录的指针。

- 如果替换成功，那么获取轻量级锁成功
- 如果替换失败，自旋。如果超过一定次数自旋之后还未获得锁，轻量级锁升级为重量级锁。

轻量级锁的释放：当前持有锁的线程将Displaced Mark Word 使用CAS复制到锁记录的MarkWord中

- 如果没有发生竞争，这个复制的操作会成功
- 如果其他线程由于多次自旋导致升级为重量级锁，那么CAS操作失败，此时会释放锁并唤醒阻塞线程。
